...
#-------------------------------------------------------------------------------
# Function to calculate Tanimoto similarity for pre-processed fingerprints
...
#-------------------------------------------------------------------------------
# Pre-process fingerprints to arrays
def convert_fp_to_array(fp_str):
    """Convert a comma-separated fingerprint string to a binary array."""
    return np.array([int(x) for x in fp_str.split(',')])

wdr91_fps = wdr91_df['ECFP4'].apply(convert_fp_to_array).values
nomination_fps = nomination_df['ECFP4'].apply(convert_fp_to_array).values
